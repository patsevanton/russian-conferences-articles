...
Т. е. Ansible-код становится таким же кодом, как код на Jave или на Python и разрабатывать его можно теми же средствами.
...
Хорошая идея, только она не пройдет, если вы пользуетесь Molecule, потому что Molecule скажет, что этот код не идемпотентный. Почему? Потому что Jython --version выполняется всякий раз неидемпотентно. Это не пропустит и Ansible-lint. И даже если вы загасите в Ansible-lint эту проверку, то потом Molecule на этапе идемпотентности вас не пропустит.
