здравствуй еще раз я александр валялся работаю в компании верта меди и мы для своих нужд разработали я разработал вас htp который включает себя реализацию штапик клиента и сервера который работает много быстрее чем стандартный этаж тебе пока избу postage т.п.
...
я думаю что многие слышали про фастов степи и сервер и ну что он себя там очень быстрый но мало кто слышал про степей клиент и вот про вначале пару слов про сервис я жил как он ну типа он участвует в бенчмарке от типа такой известный benchmark в узких кругах для htp серверов он участвует в 13 12 13 раунде и 13 раунд еще не вышел 90 раунде 

![](https://habrastorage.org/webt/ow/t8/hv/owt8hvhbrxpvfxihc1golxlgp-y.png)

вот типа результаты одного из тестов 12 раунда где пастор степи вот находится почти в самом верху это сколько он делает запрос в секунду на данном тесте тест просто делается запрос страничке это дает hello world на хорде постарше цепи очень быстро

![](https://habrastorage.org/webt/tk/h8/1t/tkh81tfzy0ee5jt0poqw_gxmv-e.png)

тут 2 результаты предварительное следующего раунда который еще не вышел но есть предварительные результаты где пассаж типе вот они 4 штуки занимают первые места в бенчмарке которые уже не только холмов дает но он еще и лезет в базу и потом формирует какойто html-страничку на основе шаблона

![](https://habrastorage.org/webt/lj/h1/ov/ljh1ovyrxst7hzjjgd16yk4tbmo.png)

ну короче fast http-сервер это круто быстрый и многим пользуются а про фастовский клиент мало кто знает но на самом деле тоже крутой и я в данную докладе расскажу вам про внутреннее устройство этого клиента и вообще зачем он был разработан

![](https://habrastorage.org/webt/z6/rp/az/z6rpazy3l_asbpcbaklgdo8mnkq.png)

в на самом деле поставь степи находится несколько клиентов и the lines хост клайн и pipeline kline дальше я расскажу подробнее про каждого из них 

![](https://habrastorage.org/webt/vs/ok/5u/vsok5umamlhuly1-gcfdewuxybu.png)

фастовский пик line это обычный http клиент для общего назначения он может с помощью него можно делать запрос любой на любой сайт интернета там получать ответы настала стандарт наш теперь клиент из фич его основной пищи это то что он кроме того что быстро работает это то что в него можно ограничивать количество открытых подключений на каждый хост в отличие от мэтта 6 пи пи джи которым количество воды крытые подключений не ограничены документация находится там на гудок можно почитать 

![](https://habrastorage.org/webt/r0/il/cc/r0ilcc3iwd6acpv1pqbawqxvfc4.png)

по степи хост клиент это специализированный клиент который для общения с одним только сервером и обычно его можно использовать нужно использовать для реализации api для обращения к htp api которая работает который работает по степи какие-то для обращения там пары крест на пик json api которая поверх http работает также его можно использовать для проектирования трафика из интернета ваш внутри нато центр нам на несколько серверов документация тут находится 

![](https://habrastorage.org/webt/sn/ys/ep/snysephpvj2u3itk5icip242ima.png)

и так же как и нет клайн просто штапик light у него есть можно ограничивать количество открытых подключений на каждый из pop up stream серверов эта пища который отсутствует в наш цепи и также эта фича отсутствует в engine и все стандартно он есть только в платном же насколько я знаю его 

![](https://habrastorage.org/webt/9b/sa/z2/9bsaz297eqozr0ckcwrlbmefdqk.png)

стаж цепи pipeline клиент это специализированный клиент с который позволяет управлять pipeline запросы на сервер но или на какое-то ограниченное количество серверов он может быть использован в для обращения к api какого-нибудь поверх http протокола там где нужно очень много из выполнить запросов в единицу времени и как можно быстрее и ограничение этого pipeline клиента в том что он страдает может страдать от head of line blocking такая штука когда мы отправляем на сервер много запросов и не ждем ответа на кажется просто и сервер на каком-то из этих запросов заблокируется кто-то начинает выполнять из-за этого все остальные запросы которые за ним пошли которые мы отправили они могут но теоретически они могли бы быстро выполнится до но они будут ждать пока этот вопрос сферу не обработает это вот есть speed of light блоки а то есть pipeline клиент нужно использовать только в том случае если вы уверены что сервер будет моментально давать ответы на ваши вопросы он не будет блокироваться деда документация там есть тут так
