...
Ответ: Если это будет актуально, это тоже добавим. Cуть этой ткани заключается в том, что она может разрастаться, если нужно. Возможно, мы обогатим каналы информацией о перетекаемых атрибутах. Сейчас это пока излишняя сложность. Потребностей особо не было.
...
Вопрос: Есть такие системы CMDB. Есть вендерные решения от IBM, HP, BMC.
...
Ответ: По подпискам по типу сообщений. У нас тип сообщений в терминологии кафка он считается топиком. И там сервис вам публикует определенные топики и мы знаем, кто на какой топик подписан. Это позволяет, если что-то кому-то не нужно, закрыть возможность, выйти на определенный топик, чтобы там что попало не слушали. 
