...
Инструменты. Что мы используем? Непосредственно сам Docker, dockerfile, в котором описывается зависимости для одного приложения. 

Docker-compose - это связка, которая объединяет самые несколько наших до к резервным приложений

Gitlab мы используем для хранения исходного кода.

Gitlab-CI и мы используем для системной интеграции.
...
Для тех кто никогда не работал с Docker и прими рисуешь эта технология которая позволяет используя класс данный подход описать необходимый компонент

Это пример урезанный пример декор файла который тем не менее является рабочим 
