...
Docker на Backend позволяет использовать интерпретатор Python, Ruby, NodeJS нужной версии. Мы избавляемся от необходимости использовать какой-то менеджер версий. Раньше для Ruby  использовали грм-пакет, который позволял менять версию в зависимости от проекта. Также это позволяет благодаря docker контейнеру плавно мигрировать код и версионировать его вместе зависимостями. У нас не возникает проблемы понять версию как интерпретатора так и кода. лишний раз и будет это допустить старый контейнер и понятного тренер
