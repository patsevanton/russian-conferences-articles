...
\01. Сплошные проблемы от вашего вакуума.

Что бывает, когда люди приходят к мысли, чтобы отключить вакуум?

Коллеги, что там с базой?!?
...
Кажется, вакуум все положил

И когда администратор дальше исследует проблему, он открывает, например, программу uitop и видит, что у него очень много запущено автовакуумов. Они все что-то делают. Они генерируют нагрузку на диск. Они создают большое количество журналов транзакций и реплики не успевают это все прожевать.
...
Оказывается, что все не очень хорошо

Все это можно быстро воспроизвести. Есть небольшой тест. Я его готовил для одной конференции. Он находится по вот этой ссылке: https://goo.gl/Tql87l
...
\02. Вакуум? Ну да, я что-то слышал про него
...
Как это выглядит на практике? К базе данных подключаются клиенты. И открывая транзакции они начинают работать с данными. Они получают снимок данных. И в этих данных они могут делать какие-то изменения: вставку новых записей и удаление существующих или обновления. 

И все эти изменения в рамках транзакции не видны другим транзакциям до тех пор, пока не произойдет commit или rollback. Как только транзакция выполняет операцию «commit», то ее изменения могут быть видны другим транзакциям, которые уже открываются или работают в данный момент. 
...
Как это выглядит на уровне страниц, на уровне строк?
...
Приходит вакуум. Он освобождает указатели и делает их доступными для повторного использования. Вычищает устаревшие версии из строк. И в итоге в странице появляется новое место, которое можно использовать для дальнейшей записи, для дальнейших inserts, updates и т. д. 
...
Как обстоят дела с вакуумом

И здесь мы уже подходим к вакууму. Как обстоят дела с вакуумом?

- Во-первых, это фоновая задача.  
- Когда мы запускаем Posgres, запускается demon автовакуума. Он периодически запускает там своих воркеров. И количество этих воркеров ограничено. Т. е. мы можем регулировать количество воркеров и сделать так, что в какой-то конкретный момент времени их будет работать максимально ограниченное количество.  
...
Как вакуум обрабатывает базу данных?
...
- Либо он выбирает те базы, где есть риск оборачивания этого счетчика транзакций. Счетчик транзакции ограничен 32 битами. Он 32-х битный и он всего 4 500 000 000. И когда в базе очень большая запись, счетчик транзакций может в какой-то момент переполниться. Для этого есть wraparound vacuum, он как раз борется с этой проблемой. Но wraparound vacuum нам сейчас не интересен.  
...
- Но Postgres не стоит на месте. Он развивается. И всякие разные тюнинги автовакуума от версии к версии тоже происходят. И наиболее значительные произошли в версии 9.6. Там много улучшений, поэтому если вы используете версии меньше 9.6 и имеете проблемы с вакуумом, то имеет смысл обновиться до версии 9.6.  
 ...
 \03. Ну Ok, давайте затюним вакуум
...
Пауза определяется параметром «delay». Тут уже понятно, что мы с помощью cost’а можем влиять на размер пачки. А с помощью delay можем влиять на интервал сна. 
...
- На современных серверах, когда количество процессорных ядер уже переваливает за 32 и более, этот параметр следует увеличивать в большую сторону. И, на мой взгляд, оптимальным значением является примерно 10-15 % от общего количества ядер.  
- Следующий параметр «naptime». Он определяет, как часто нужно запускать autovacuum worker. По умолчанию он равен 60 секунд. И это довольно большое значение. Имеет смысл его уменьшать до 1 секунды, до 2 секунд, до 5 секунд. Опыт показывает, что одной секунды достаточно с точки зрения кода postgres’ового. Там небольшая функция, которая отсчитывает таймер и смотрит – не пора ли нам запустить вакуум. Т. е. это не ресурсоемкая операция. Имеет смысл всегда ее уменьшать.
- И другой важный момент – это то, что параметр, который влияет на размер пачки делится всегда между теми активными воркерами, которые запущены прямо сейчас. Это тоже следует помнить, cost_limit следует делить между всеми воркерами.  
...
И рассчитывается она таким образом. Мы берем количество строк в таблице с момента последнего выполнения вакуума и умножаем на переменную «scale_factor». Это, условно говоря, такое дробное число. Это процентное отношение. Допустим, по умолчанию это 0,2, т. е. 20 %. 

К полученному числу мы добавляем значение «threshold». Там по умолчанию – это 50 строк. И получаем тот самый порог, выше которого нужно запустить автовакуум. 

Таким образом вакуум запускается по умолчанию, если мертвых строк в таблице более 20 %. На больших таблицах это очень большой порог, поэтому всегда имеет смысл его уменьшать и делать, например, 1-2-5 %. 

Что использовать?

И бывают ситуации, когда регулирование с помощью scale_factor не приносит должного эффекта. Особенно это на больших таблицах себя проявляет. Когда внутри таблицы миллионы строк, то даже 1 % - это очень большое количество строк. Тогда имеет смысл поставить scale_factor в 0. И пользоваться только threshold параметром, т. е. явно указывать, что запустить вакуум после одного миллиона строк. Когда у нас появилось миллион мертвых строк в таблице, то нужно запустить вакуум. Нужно выбирать: либо мы scale_factor используем для каких-то индивидуальных таблиц, либо threshold для них. 
...
И если вы используете NVME диски, то проблемы вакуума как таковой вообще нет, потому что это очень производительные устройства. И там вопрос I/O практически никогда не стоит. Там можно делать агрессивный автовакуум, делать много воркеров и вообще не париться. Единственное, что нужно настроить мониторинг и все-таки следить – нет ли каких-то проблем с автовакуумом в процессе работы базы. Поэтому чем лучше оборудование, тем проблемы ОТО, связанные с вакуумом, становятся меньше.

Может есть универсальное правило?

И здесь можно вывести какое-то общее правило. Мы регулируем delay. Это интервал сна между обработкой и размером пачки. Т. е. всегда нужно отталкиваться от этого. Вы регулируете размер пачки. Регулируете интервал. Смотрите мониторинг. Смотрите – есть ли у вас проблемы или нет. 
...
И бывают случаи, когда автовакуум простой не помогает, поэтому приходится прибегать к каким-то сторонним средствам, которых нет в штатной поставке Postgres. Это, как правило, утилиты pgcompacttable и pg_repack.
...
Да. Ph_cache она вымывает. Данные появляются в ph_cache, а шаредный буфер она не вымывает. Все верно. Ph_cache вымывается.
...
*Спасибо за доклад! Предположим, что у нас есть две открытые транзакции. В одной из транзакций мы изменяем данные: удаляем или обновляем. Автоматически спустя какое-то количество* *scale**_**factor* *или* *threshold* *начинает выполняться как* *…* *При этом записи он не может освободить, потому что открыта вторая транзакция, которая снапшот, которая видит еще эти строки. И в итоге получается, что* *naptime* *секунд автовакуум портит статистику, если выполняется без* *analyze**. Причем в текущей версии* *Postgres* *в каждой статистике добавляется еще количество мертвых строк. Это отражается в* *pg_class_rel**…*
...
*Для того чтобы не добавлялось к этому значению количество* *…* *tuples* *в* *community* *я нашел патч, который сделали разработчики 2**ndQuadrant**. Протестировал – работает. Но тем не менее каждый вакуум без* *analyze* *увеличивает количество* *real* *tuples* *и портит статистику.*

Да, портит статистику. Основная идея – это избегать долгих транзакций. Уменьшить их длину жизни. Следить за тем, чтобы они не были в статусе «idle transactions», чтобы база данных не ожидала, когда придет следующая команда от клиента. Не делать в транзакции какие-то обращения к внешним ресурсам. Вот это основная рекомендация, т. е. уменьшить время жизни транзакции и следить, чтобы они не тупили. 
...
*Зачем каждый раз увеличивать количество* *real* *tuples**?*
...
*Мне пришлось бороться вот с чем. У нас было несколько сотен таблиц, в каждой из них было по несколько сотен строк*. *И спустя какое-то время мы увидели, что какой-то вакуум стал доходить до абсолютного лимита. Одному из вакуумов потребовалось двое суток, чтобы вернуться к равновесию. У вас есть какие-то представления на этот счет, как этот вопрос решить?*

Вы подходите к нашему стенду, мы это обсудим, потому что плохо слышно. 
