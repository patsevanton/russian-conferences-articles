...
*Привет! Меня зовут Лев. Есть вопрос: «Вы тестируете роли свои и пользуете* *lintes: какой-нибудь* *YAML,* *Ansible-lint или фреймворки типа Молекулы?».*
...
Проще эту задачу решать через <font color="red">setting</font> defaults, через нормальные дефолты и переменные одного уровня, т. е. не плодить словари, а делать переменные в один список. 
...
Везде, где это не критично, если мы не говорим о запуске System D, хотя даже там можно обойтись <font color="red">судо</font>, обычно все сценарии не из-под root. 

*Привет! Вы, наверное, используете в своих ролях* *include-таск, import-таск? Если используете, то как к этому относитесь, потому что получается, что одна роль зависит от другой или от каких-то общих вещей? И есть ли у вас роль* *command?*
