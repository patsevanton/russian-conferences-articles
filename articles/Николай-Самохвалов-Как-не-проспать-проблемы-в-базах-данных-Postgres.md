...
![](https://habrastorage.org/webt/lk/vi/ql/lkviqldxswpgjf-74bzy1dodf28.png)
...
![](https://habrastorage.org/webt/w5/w_/mu/w5w_muxrt-02l0e0f6b4pzwizow.png)
...
Есть другие инструменты. Некоторые из них здесь перечислены. Например, pghero бесплатный, написан на Ruby. Он очень легкий и простой, но хорошо закрывает некоторые вопросы. 
...
Относительно свежая история с <font color="red">Mem…</font>, когда они пролежали полутора суток. Была деградация. Один шард полностью лежал, потому что они уперлись в transaction id wraparound. И это проблема очень болезненная. 
...
Еще бывает специальный случай, когда индекс нам особо не нужен, но он поддерживает внешний ключ. У нас для этого есть колонка. Здесь ни один индекс не поддерживает внешний ключ, т. е. со одного конца внешнего ключа индекс обязателен для Postgres, а с другого конца не обязателен. Reference table, т. е. откуда мы ссылаемся. И он обычно как бы не нужен, но если вы будете эту строчку удалять, то там вы получите <font color="red">… scan</font> в полной таблице, потому что он должен убедиться, что на него никто не ссылается. 
...
И тоже самое мы делаем для всех других метрик. Мы видим, что, допустим, в первой группе работа с кэшом операционной системы составляет 7 % от всей работы с кэшом операционной системы. 
