...
В заключительной части доклада рассмотрим практические примеры применения [clickhouse-operator](https://github.com/Altinity/clickhouse-operator) с Amazon или Google Cloud Service. Доклад строится на примере разработки и опыта эксплуатации оператора для ClickHouse.

кат

Меня зовут Владислав Клименко. Я хотел сегодня рассказать о нашем опыте разработки и эксплуатации оператора, причем это специализированный оператор для управления кластерами баз данных. На примере [ClickHouse-operator](https://github.com/Altinity/clickhouse-operator) для управления кластером ClickHouse.
...
![](https://habrastorage.org/webt/dp/wg/7z/dpwg7z3v6jc3f2u9jwakplubwpy.jpeg)По Persistent Storage у нас есть различные варианты исполнения.
...
И что для этого надо сделать? Во-первых, на сцену выходит Custom Resource Definition. Что это такое? Это описание K8s, что у тебя будет еще один тип данных, что мы к pod, StatefulSet хотим добавить кастомный ресурс, который будет сложным внутри. Это описание структуры данных. 
...
Но пока что ничего дальше не произойдет. Т. е. если мы сейчас создадим YAML-файл, который мы рассматривали с описанием шард, реплик и скажем «kubectl apply», то Kubernetes его примет, положит в ETCD и скажет: «Отлично, но что с ним делать, не знаю. Как обслуживать ClickHouse-installation я не знаю». 
...
Он это все подготовил и передает дальше в K8s. Говорит, что ему надо ConfigMap, StatefulSet, Volume. Kubernetes отрабатывает. Он материализует базовые единицы, которыми он апеллирует. 
...
Но если у нас велик процесс апдейта кластера или у нас кластеров много, или постоянно что-то меняется, то процесс динамический. И постоянно заниматься перенастройкой мониторинга – это трата ресурсов времени, т. е. просто лень. Это надо автоматизировать. Сложность именно в динамике процесса. И оператор это очень хорошо автоматизирует. 
...
*Здравствуйте! Спасибо за доклад! Дмитрий Завьялов, компания* *Смедова. Планируется ли добавление в оператор возможности настройки с hproxy? Интересен какой-нибудь другой балансировщик помимо стандартного, чтобы он умным был и понимал, что там реально ClickHouse.* 
