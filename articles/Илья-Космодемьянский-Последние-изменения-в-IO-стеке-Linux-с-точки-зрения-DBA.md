...
Я прошу прошение, что слайды на английском, говорить я буду на русском, но в нашем деле обычно очень много английской речи, потому что изменения происходят настолько быстро, что не все успевает попасть в язык. 
...
И основная зада современной базы данных – это странички с диска подымать в память. И когда у нас произошло какое-то изменение, помечать эти странички как грязные, записывать writes add a log. И после этого синхронизировать память, чтобы она была консистентна диску. 

В ситуации с Postgres – это постоянно путешествует туда-сюда из шаредной памяти, которая менеджится Postgres в Page cache ядра и далее на диск через весь линуксовый stack.

С той или иной разницей, если вы используете базу данных на файловой системе, она у вас будет так работать более-менее с любой UNIX-подобной системой и с любой базой данных. Если вы используете Oracle, то у вас будет немножко другая история. Oracle сам будет взаимодействовать с диском, но принцип такой же. Задача – как можно быстрее прососать эти странички через весь stack ввода-вывода, каким бы он не был. 
...
Вторая сторона, с которой часто сталкиваются DBA, это затык записи в writes add a log, когда у нас настолько мощная нагрузка, что даже последовательно записываемый журнал, упирается в диск. И в этой ситуации его тоже надо записать быстро. И фактически она мало чем отличается от ситуации синхронизации кэша. С той только разницей, что там меньше приспособлений для этого. С Postgres работаем с большим количеством шаредных буферов. В базе есть механизмы, как эффективнее это списывать на диск. А writes add a log он и без этого до предела оптимизировал. И единственное, что мы можем сделать, чтобы сам log писался эффективнее, это только поменять настройки Linux, чтобы он эффективнее писал. 

Key thing about modern database workload
...
What generates most IO in case of PostgreSQL
...
Но когда автовакуум настроен вот так агрессивно, он у вас начинает жевать IO. И вот эта вся радость еще может накладываться с checkpoints, и у вас большую часть времени диски утилизованы почти на 100 %. И это все источник проблем. 
...
The main IO problem for databases for a long time was
...
Because of high latency of rotating disks
...
Elevators: before 2.6 kernel

До того, как было 2.6 ядро, был Linus Elevator. Это был самый простой IO Scheduler. Поэтому долгое время он считался абсолютно незыблемым и очень хорошим. Но по мере того, как один человек по-прежнему его считал хорошим, люди стали новые разрабатывать. 

И он имел много проблем, потому что merging и sorting в зависимости от того, как эффективнее писать. И это в случае с вращающимися дисками приводило к тому, что возникала такая ситуация starvation. Если у нас есть диск, чтобы на него эффективно записать, его нужно повернуть так. А теперь нам внезапно с него нужно одновременно эффективно прочитать, а он уже так повернут. И, соответственно, читается с такой штуки плохо. 

Elevators: between 2.6 and early 3.*
...
Тут возникает некая путаница. Я поэтому заранее скажу, что очень многие путают эти schedulers с scheduler операционной системы, потому что у них похожие названия. 
...
Какая идея? Идея такая – давайте использовать для ввода-вывода нативную возможность SDD делать эффективный параллелизм. Это и произошло. Там, в зависимости от того, сколько мы параллельных потоков ввода-вывода можем использовать, у нас есть очереди. Очереди честные. И мы через эти очереди пишем issues на наши SSD. И для каждой CPU – это своя очередь для записи.  

В настоящий момент эта штука очень активно развивается, работает. И сейчас нет причин не использовать это дело, т. е. в современных 4 ядрах выигрыш от blk-mq может быть очень большим. Это будет ощутимо. Это не 5-10 %, это больше. И сейчас мы считаем, что это лучшая опция для работы с SDD.
...
Более какие-то продвинутые elevators работали так: у нас есть несколько SPU, есть несколько очередей. И каким-то образом, например, в зависимости от какого родительского процесса отпочковались наши workers, идет в какие-то очереди и попадает на диски. 
...
Если кто-нибудь обращал внимание, все вендоры RAID-массивов стали продавать такие done, которые позволяли, если у вас подключены SSD, напрямую писать туда. Фактически они тоже самое делали. Они отключали применение SSD-драйвера для своих продуктов. 

IO stack (with blk-mq)
...
И много мелких вещий по улучшению Direct IO. Но тут сразу возникает вопрос, что, например, в Postgres Direct IO сейчас не поддерживается. И есть ряд проблем, которые мешают включить поддержку этих вещей. Т. е. сейчас это только для вала и только, если у нас не включена репликация.

Notes on Direct IO

Но поскольку очень много специфичного кода надо написать, то пока что все с этим делом воздерживаются. И несмотря на то, что … сильно ругается на эту идею и на то, как она сделана, но, в принципе, все базы данных туда идут. Сейчас в MySQL и в … это активно используется. 

На этом у меня все. Если вы хотите задать вопросы, то задавайте. 
...
*Здравствуйте! Меня зовут Дмитрий. Если использовать* *b**_**cache* *и базу данных поверх этого* *b**_**cache**, то какой* *scheduler* *лучше всего использовать?*
