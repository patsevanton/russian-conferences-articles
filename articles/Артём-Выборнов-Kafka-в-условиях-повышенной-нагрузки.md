...
У нас был worker. На самом деле их было много, просто здесь на рисунке изображен один. Он забирал на себя данные, локально их сохранял, а затем перекладывал в Hadoop. 

![](https://habrastorage.org/webt/tf/wz/nh/tfwznhle0anme9z5ibnvsy1ohe4.png)

Данные мы храним в HDFS и обрабатываем с помощью Spark, Hive. После того как они обработаны, нам требуется полученной информацией поделиться с окружающим миром - под это мы используем key-value базу Aerospike.
...
- Во-вторых, worker здесь выглядит как один сервер. На самом деле это какой-то набор воркеров и каждый файл должен сначала пройти через одну из нод, где будет сохранен на диск, а потом уже выгружен в Hadoop. Это узкое место.
- В-третьих, таким способом нельзя реализовать real-time pipeline обработки данных. Предположим нам какой-то партнер стал присылать данные микробатчами по одной минуте. Мы их забрали и сохранили на сервере, переложили в HDFS, на HDFS обработали, выгрузили в Aerospike. Если всё пройдет идеально, то это задержки в десятки минут, - ни о каком real-time речь уже не идет.

![](https://habrastorage.org/webt/6f/wb/tu/6fwbtuiahk4mxcowmxe0qszukxq.png)

Что мы сделали? Мы все, что находится слева, выбросили и заменили на Kafka. Kafka – это брокер сообщений для больших данных. Он не обладает такой крутой логикой доставки сообщений, как RabbitMQ. Он более простой, но при этом он более производительный при больших нагрузках.

В Kafka могут писать много поставщиков и из неё могут читать много потребителей. 
...
Также у нас появилась возможность приблизиться к real time. 
...
Самое главное преимущество такого подхода в том, что мы полностью минуем стадии записи и чтения из HDFS, все происходит в памяти. Мы с Kafka считали, обработали, положили в Aerospike. 

Такой подход привёл нас к задержке между наступлением события и обновлением соответствующей информации в Aerospike порядка одной минуты. 
...
Kafka состоит из наборов брокеров (каждый брокер – это отдельная машина) и ZooKeeper для хранения метаданных. На самих брокерах хранятся сообщения. В Kafka пишут producers, из Kafka читает consumers. 
...
Если вы ранее слышали про Kafka, то наверняка знаете, что одним из бонусов Kafka и отличием от других популярных решений является то, что она хранит данные на диске и вдобавок их реплицирует. 
...
Все бы хорошо, но у асинхронной репликации есть типичная проблема - slave-партиция может отставать от мастера (на некую дельту). Это в целом не так уж и страшно, но стоит обратить на внимание на процесс переизбрания лидеров. 
...
Если slave в момент переключения отставал от мастера больше, чем на какое-то отдельно определенное число сообщений, то мы говорим, что это unclean, а если меньше, то говорим, что это clean. 
...
А также есть еще один неочевидный нюанс, который не описан в документации: переизбрание может происходить, когда у вас все хорошо. Ваш кластер Kafka работает стабильно, нагрузка распределена равномерно, время ответа небольшое, а Kafka взяла и переизбрала лидеров, и следовательно мы потеряли данные. 
...
Интересный момент. Если значение этого параметра -1, то Kafka вышлет подтверждение, при успешной записи во все синхронные партиции. А что Kafka понимаем под синхронными партициями - конфигурируется отдельным ключом на самой Kafka. Вы для каждого топика можете задать число партиций, которые Kafka должна держать в insync. И если insync стоит 1, то это никак не отличается от предыдущего примера. 
...
Более серьёзная проблема заключается в том, что если в данной ситуации одна из реплик выпадает, то мы вообще не сможем продолжить запись данных в топик, потому что Kafka ждет, что будет записаны две реплики, а доступна только одна. 

![](https://habrastorage.org/webt/4x/pe/st/4xpesttbddpc1oclduxyl3el5ie.png)

Это довольно серьёзная проблема. Сейчас я опишу подход, к которому мы пришли, насколько он хорош - уже вам решать. Мы держим для важных топиков insync.replicas = 2, а replication factor = 3. При таких настройках если у нас одна из insync реплик выпадет, то оставшаяся реплика за конечное время догонит лидера, и продюсер сможет продолжить запись к нам в Kafka. 
...
Как вы помните, партиция представляет собой набор пронумерованных сообщений. И отступ как раз и есть номер этого сообщения. 
...
Kafka хранит данные не все время, а за какой-то период, который вы задаёте в настройках, и потихоньку в фоне удаляёт слишком старые данные. Поэтому earliest offset постоянно движется вперед. По сути, между earliest и latest offset – мы получаем окно, которая постоянно движется во времени вперёд.

![](https://habrastorage.org/webt/fm/de/ih/fmdeihpdeo07tiq6kpjbb4uobcu.png)

Предположим у нас ранее отработал consumer и зафетчил данные, вплоть до какого-то отступа, т. е. данные от earliest offset до offset у нас уже были зафетчены и куда-то сохранены. При следующем запуске нашего consumer требуется забрать выделенный розовым кусочек данных с offset до latest offset и обновить отступы (то есть значение отступа до которого мы выкачали данные). 
...
Подход по умолчанию – автоматическое сохранение. Но на самом деле он самый проблемный.

Как это работает? Consumer фетчит данные из Kafka. Kafka периодически до него стучится – жив он или не жив. Если он жив, то Kafka сохраняет соответствующий отступ. 

В чем здесь проблема? К примеру, consumer зафетчил данные, Kafka проверила то, что он жив, сохранила отступ, а он не успел записать данные на диск и упал. Мы получили потерю данных. 

А может быть и другая ситуация. К примеру, consumer успел записать данные на диск и упал, Kafka пришла проверить жив ли он, а он уже мертв. Kafka отступ не сохранит, а мы получим дубли в данных. Т. е. такой подход не способен обеспечить никакую семантику доставки. 

![](https://habrastorage.org/webt/yd/1p/y1/yd1py1sxbbl5jbeidltdoujkwty.png)

Следующий способ – это сохранение вручную. Сonsumer фетчит данные из Kafka, после скачивания очередного батча, он говорит: «Kafka, я этот кусочек данных успешно скачал, сохрани отступы». Вроде бы все хорошо, но это обеспечит только at least once семантику. 

At least once – это такая семантика, при которой мы данные не теряем, но могут быть дубли. 

Почему так может произойти? Рассмотрим аналогичную ситуацию: предположим, мы сохранили батч данных, но, когда consumer начал отправлять уведомление об этом Kafka, кто-то дернул `kill -9` и consumer благополучно упал, не успев сообщить о том, что нужно было сохранить отступы. Получили дубли данных. 
...
Единственный путь обеспечить exactly once на чтение – это хранить данные и отступы вне Kafka. При этом сохранение данных и отступов должно быть атомарно. Потому что, если оно будет не атомарно, то мы можем получить проблему аналогичную предыдущему примеру: мы сохранили сколько-то данных, а отступы потерялись. И на следующей итерации мы повторно их выкачаем.
...
Предположим, первый отработал успешно, а на втором мы упали. Тогда при следующем запуске consumer’а, он первым делом посмотрит в эту временную директорию и узнает какие файлы еще не были перемещены. Также у него есть информация о том, что должно было быть перемещено и куда. На основе этих вводных он попытается завершить этот процесс. 

Если у него это получилось - значит мы закончили предыдущий fetch данных при начале следующего fetch’а. Если не получилось завершить, то вся необходимая информация для полного отката изменений предыдущего фетча также присутствует. Таким образом можно получить exactly once даже при сохранении в базах данных без транзакций. 
...
Все отлично, но у нас начались проблемы. Поговорим о том, какие именно проблемы мы обнаружили.
...
Если данные за вчера доезжают, к примеру, сразу после полуночи (в 1-2 ночи, где зеленая отметка), то все зависящие от них pipeline успешно отрабатывают.
...
Кроме этого, мы заметили очень медленную работу consumers на микробатчах: минутный батч данных из Kafka фетчился по 2-3 минуты.
...
Но еще более интересную картину мы увидели, когда посмотрели на эту же информацию (нагрузка на чтение) в разрезе нод. Здесь просто какая-то лапша которую сложно разобрать.
...
Следующее решение совсем не очевидно - повышаем репликацию. Хотя казалось бы: трафик на чтение увеличился в 4 раза, зачем его еще увеличивать чтением на синхронную репликацию?
...
Весь трафик переезжает на другой брокер и плохо уже ему. И именно эту картину вы и видели на графике в разрезе по нодам.
...
И корень этой проблемы заключался в следующем. Мы использовали батчевый consumer данных с Kafka - Camus. Это предок Gobblin. 

![](https://habrastorage.org/webt/by/tb/di/bytbdi6snb37k1k6evsiv-yx2ce.png)

И он сохраняет отступы для топиков в директорию под названием history. И отступы всех топиков попали в одну папку. Вот здесь приведен фикс, исправляющий проблему. И, по сути, все отступы попали в одну директорию. 
...
Это нагрузка на Kafka в нормальном режиме работы. В 4 раза упал пиковый трафик. А вот эти периодические пики, которые вы видите, это запуск нашего consumer раз в полчаса. Т.е. у нас на Hadoop работают задачи, которые каждые полчаса просыпаются, фетчат новые данные и обратно засыпают. 
...
Здесь стоит отметить, что мы сейчас уже держим нагрузку большую, чем та при которой у нас были ранее описанные проблемы, но при этом все хорошо. Растем дальше. 
...
В-третьих, стоит мониторить рассинхронизацию партиций. В любой, наверное, системе, где есть асинхронная репликация, на это стоит смотреть. Метрик для этого целое множество. Из тех, которые нравятся мне, это:

- Число несинхронных партиций из тех, которые должны быть in sync.
- Максимальный лаг репликации. 

Это график максимального лага репликации с точностью до нод, на сколько сообщений мы отстаем:

![](https://habrastorage.org/webt/8x/nj/tf/8xnjtfnudaaldbmj6xmfqggjbem.png) 

Что по этому графику видно? Все хорошо, лаг в пределах нуля с отдельными пиками. И это нормальная ситуация. Так все хорошо. 
...
И красной линией выделено отсечка в 10 секунд. Мы сейчас ее используем. Она эмпирическим образом найдена. Если Kafka отвечает меньше, чем за 10 секунд, наши обработчики данных работают хорошо. Если Kafka отвечает больше, чем за 10 секунд, кричим: «Караул!».
...
- Нужно мониторить узкие места. В нашем случае при повышении нагрузки "первое, что у нас отвалилось, это сеть, диски и баланс нагрузки по нодам. 
