...
Мы данные храним в HDFS. Обрабатываем на Spark, Hive. После того как они обработаны, мы хотим полученной информацией поделиться с окружающим миром. Нам нужна для этого key-value база. Мы используем Aerospike. 
...
- Во-вторых, нода здесь обозначена как сервер. На самом деле это какой-то набор воркеров, но каждый файл должен был сначала пройти через одну ноду, где будет сохранен на диск, а потом уже будет выгружен в Hadoop. Это узкое место.
...
Что мы сделали? Мы все, что находится слева выбросили и заменили на Kafka. Kafka – это брокер сообщений для больших данных. Он не обладает такой крутой семантикой и доставкой сообщений, как RabbitMQ. Он более простой, но при этом он более надежный. 
...
Это решение от создателей Kafka компании LinkedIn. Это MapReduce задача, которая крутится на Hadoop. И данные из Kafka кладет в HDFS. 
...
Каждую партицию можно представить как Array list. Это какая-то последовательность сообщений, где все сообщения пронумерованы, причем чем сообщение свежее, тем больше у него номер. 

![](https://habrastorage.org/webt/rf/wz/ht/rfwzhtk0h0pw-xwmaqgmpltwc-4.png)

В партиции дописывает продюсер. И по партициям дополнительно сообщения шардируются. 
...
Репликация в Kafka происходит с точностью до партиций. Репликация асинхронная: master-slave. Мастер в терминах Kafka называется лидер. 

![](https://habrastorage.org/webt/lu/wx/0z/luwx0zfeso85_lzxhzk6brs7ddc.png)

Все consumers, producers всегда работают только с лидером, т. е. пишут и читают только с лидера. И потом данные с лидера aсинхронно доезжают до всех slaves.
...
В документации по Kafka есть 2 вида переизбрания лидеров: это clean и unclean. Кажется, что unclean – это какой-то плохой способ, т. е. мы тогда теряем данные, а clean – это хороший, мы тогда не теряем данные. Но на самом деле это не так. Между ними различие в размере этой дельты. 
...
Очень важно понимать, что при переизбрании лидеров мы всегда теряем данные.
...
Вроде бы все хорошо. В таком случае, если будет переизбрание лидеров у нас не будет никаких проблем с потерей данных, потому что все синхронно. Да, бесспорно, будет какой-то overhead на продюсер, т. е. он будет просто дольше ждать ответа от Kafka, но это не так страшно. 
...
И это довольно плохо. Мы используем для этого такой подход. Насколько он хорош - уже вам решать. Мы держим для важных топиков insync.replicas = 2, а replication factor =3 . И если у нас одна из insync реплик выпадет, оставшаяся реплика сможет догнаться, и продюсер сможет продолжить писать нам в Kafka. 
...
Latest offset – это номер самого свежего, последнего добавленного в партицию сообщения. 
...
Предположим у вас отработал какой-то consumer. Он зафетчил сколько-то данных, вплоть до какого-то отступа. Все хорошо, т. е. данные от earliest до offset у нас уже были зафетчены и куда-то сохранены. При следующем запуске вашего consumer вам нужно будет забрать вот этот выделенный розовым кусочек данных с offset до latest offset и сохранить отступы. 
