...
Для этого существует несколько способов, когда вы выбираете consumer для Kafka - это первое на что вы должны обратить внимание. 

Самый базовый путь – автоматическое сохранение. Но на самом деле он самый плохой.
...
Почему так может произойти? Точно такая же ситуация. Предположим, мы сохранили кусок данных, но, когда мы делали данные, кто-то дернул `kill -9` и ваш consumer благополучно упал, не успел сообщить Kafka, что нужно было сохранить отступы. Получили дубли данных. 
...
Единственный способ обеспечить exactly once – это сохранять и данные, и отступы вне Kafka. При этом сохранение данных и отступов должно быть атомарное. Потому что, если оно будет не атомарное, то получаем ту же самую проблему. Мы могли сохранить данные, а отступы потерялись. Мы повторно их выкачали.
...
Предположим, у нас отработал consumer и сохранил 2 файла с данными и файл с отступами в какую-то папку. Затем их надо переместить в какую-то production-директорию. Move в Hadoop атомарный.
...
И если у него это получится, то все хорошо. Мы закончили предыдущий fetch данных при уже следующем fetch. Если это не получается, мы можем откатить. Таким образом можно обеспечить exactly once при сохранении в базах данных без транзакций. 
...
Во-вторых, у нас появились дубли сообщений. Эта пресловутая семантика Exactly once, которой мы так дорожим, была нарушена. 
...
И причина была общей для этих двух проблем: Kafka стала очень медленно отвечать на простейшие запросы - по несколько минут. Точнее отдельные ноды Kafka стали очень медленно отвечать на эти запросы. 

И когда мы фетчим получасовым батчем информацию, то эти пару минут можем даже не заметить, потому что надо запуститься раз в полчаса. А если мы фетчим каждую минуту, то эти две минуты будут намного более заметней. 
...
Как мы ее решали? Более-менее автоматизированного способа решения мы не нашли. Единственное решение, которое мы нашли – это распределять руками. В Kafka можно скормить конфиг, в котором можно описать, на каких нодах должны лежать какие-то партиции и какие из них должны быть лидерами. Это позволит построить картинку ближе к идеалу. 

Понятное дело, все, о чем я говорю, верно для самых толстых топиков, через которые у вас проходит наибольшее количество информации. 
...
Все, что я описываю верно для Kafka 0.8.2. Возможно, в новых релизах сделали ее чуть умнее. Я на это очень надеюсь. Но для старых версий это очень актуально. 
...
- Мы уперлись в сеть. У нас часть нод в стойке оказались с 1 Gb uplink. Это примерно 125 мегабайт в секунду. На графике у нас на ноду приходилось более 150 мегабайт в секунду. И понятное дело, сеть была загружена. 
- Мы внезапно узнали, что Kafka не дружит с RAID 5. 

Почему так получилось? Документация читается в последний момент. Если у вас RAID 5 и все работает хорошо, то у вас все хорошо. Но когда у вас на RAID 5 теряется какой-то блок данных и начинается восстановление, в этот момент производительность Kafka резко просаживается. Для Kafka рекомендуется использовать RAID 10.
