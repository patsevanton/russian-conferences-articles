...
Вот ссылка. Он тоже опубликован. Вы можете посмотреть, как это работает внутри. Это демонстрационный вариант того, как можно писать свои сложные actions, которые ходят в API и делают всякое. 
...
Что это такое? Давайте посмотрим. Заходите в Danger, в репозиторий. 
...
Следующая штука – это вообще огонь – это Dependabot. Я везде его использую, всем его советую. Без него я уже не знаю, как жить. 
...
Дальше нам нужно что-то релизить. И когда мы раньше релизили, у нас было много боли, потому что релизили мы иногда с ошибкой версии, иногда забывали change log написать. В общем, бывало всякое. Я думаю, что у всех у нас такое было. 
...
Дальше мы устанавливаем action, который занимается непосредственно запуском этого благородства. 
...
И потом находим имя секрета. Указываем имя секрета, значение секрета. Готово. 
...
![](https://habrastorage.org/webt/vb/jg/zp/vbjgzp4xy4hikoa2muu6xfsjul4.jpeg)

Для того чтобы его установить, заходим. 
...
- Rsync. Можно даже Rsync использовать, уже есть готовый action для этого. 
- Docker registry. Можно прямо в Docker registry все это релизить, если у вас дальше pipeline настроен от этого места. 
...
Вот ссылка на официальную штуку для Terraform. Она поддерживается HashiCorp. И я думаю, что можно ею смело пользоваться.
...
Я не знаю, я с Big Basket 100 не работал. 
...
*Ты показывал кастомный UI <font color="red">…</font> и у тебя появилась какая плашка, кнопка. Действия на эту кнопку тоже что-то триггерят. Там есть отдельная библиотека, чтобы рисовать эти UI и компонеты, либо как происходит это кастомное рисование внутри репозитория, когда ты action подключил?*
...
*Как защититься от открытия секретов, если в <font color="red">Per action в YAML написать echo</font> secrets?*
