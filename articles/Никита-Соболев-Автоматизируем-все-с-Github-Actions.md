...
Ребята, привет! Меня зовут Никита Соболев. Я технический директор компании «We make services». У меня есть несколько традиционных вопросов для вас. 
...
- И нас есть свои собственные боты. Это большие приложения. Некоторые из них, действительно, большие приложения, некоторые из них чуть поменьше. Это монолиты, которые слушают вебхуки и живут непонятно где. Я это все поддерживаю, у нас есть инфраструктура. Один даже в Kubernetes живет. Кстати, у нас сегодня будет детокс от Kubernetes. Это единственный раз, когда я про него сказал и все.
...
Как мы будем это делать? Вот наш проект. Мы заходим в него. 
...
![](https://habrastorage.org/webt/hp/6d/wf/hp6dwfdlgu2g-ycxsu6stl09ihi.jpeg)
...
Но есть варианты выбора из Linux, MacOS и Windows, т. е. на всех трех платформах можно запускать. Если вы запустите на трех, то у вас появится матричная сборка, т. е. matrix build.
...
Версия 2.7 скоро умрет, не проверяйте ее. 
...
В первом мы говорим: «Пожалуйста, установите мне flke8 (это линтер для Python) и прогоните его».
...
Мы можем указать список параметров, которые мы ожидаем. Помните, что я говорил, что у меня есть входной параметр, который называется «путь»? Вот здесь я его определяю. У меня путь – это список путей, которые я проверяю. Он необязательный, по умолчанию точка, т. е. дефолтная директория. 
...
- Это GitHub core, который работает со всякими входными и выходными параметрами. 
- Exec – для того, чтобы запускать бэкграунд-задачи. 
- IO – для того, чтобы создавать файлы и папки.
- Tool-cache – для того, чтобы создавать cache.
- Github – для того, чтобы общаться с API GitHub.
...
Давайте посмотрим, как это делается. Сверху мы пишем в java script’е два import: github и core. 
...
«А если есть, то, пожалуйста, обнови комментарий, который равняется Input дефолтному с именем «текст»». 
