...
![](https://habrastorage.org/webt/si/lv/db/silvdbdioopkwhikvfrsksppaje.jpeg)
...
![](https://habrastorage.org/webt/lr/ms/7x/lrms7xwzzepfnseq0wroozfa69a.jpeg)
...
В этом случае, если вы отправите SELECT, то будет сделан запрос в мета-данную в ZooKeeper. И SELECT будет разрешен только на репликах, которые консистентные, т. е. содержат все раннее подтвержденные INSERT, записанные с кворумом. 
...
Примечание: на 26 минуте 35 секунде демо.
...
Примечание: на 27 минуте 30 секунде демо.
...
Примечание: на 29 минуте 5 секунде демо.

Давайте проверим. Выставляем max_memory_usage в 10 гигабайт. Сбрасывать будем 8 гигабайт (max_bytes_before_external_group_by = 8 000 000 000, distributed_aggregation_memory_efficient = 1). И у нас progress-bar завис на какой-то момент. А потом дальше продолжил идти. Что это значит? Это значит, что именно в этот момент времени данные сбрасывались на диск. И как не странно, запрос обработался даже не сильно дольше, чем запрос без этих настроек. 
...
![](https://habrastorage.org/webt/tw/k1/io/twk1ioozjoqqvch1kfhcadkhs98.jpeg)
